...
// --- context list ---
export function formatContextList(contexts, json) {
    if (json) {
    ...
    for (const ctx of contexts) {
        ...
        lines.push(`  ${padEnd(ctx.name, maxName)}  (${agg} aggregate${agg !== 1 ? "s" : ""}, ${terms} term${terms !== 1 ? "s" : ""})`);
    }
    return lines.join("\n");
}
// --- context show ---
function formatAggregateOneLiner(agg) {
...
export function formatContextShow(ctx, json) {
    if (json) {
        return JSON.stringify(ctx, null, 2);
    }
    const lines = [`Context: ${ctx.name}`, ""];
...
// --- aggregate show ---
export function formatAggregateShow(agg, contextName, json) {
    if (json) {
        return JSON.stringify({ ...agg, context: contextName }, null, 2);
    }
    const lines = [`Aggregate: ${agg.name}`, `  Context: ${contextName}`, ""];
    if (agg.root) {
...
// --- glossary ---
export function formatGlossary(ctx, json) {
    if (json) {
        return JSON.stringify({ context: ctx.name, glossary: ctx.glossary }, null, 2);
    }
    if (ctx.glossary.length === 0)
        return `Glossary: ${ctx.name}\n\n  (empty)`;
    const lines = [`Glossary: ${ctx.name}`, ""];
    const maxName = Math.max(...ctx.glossary.map((t) => t.name.length));
    for (const t of ctx.glossary) {
        lines.push(`  ${padEnd(t.name, maxName)}  — ${t.description}`);
    }
