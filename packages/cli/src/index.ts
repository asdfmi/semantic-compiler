...
import type { RawContext, NormalizedContext } from "./types.js";
...
function loadContexts(dir: string): NormalizedContext[] {
  const root = resolve(dir);
  const files = walkFiles(root, "definition.ts");
  const rawContexts: RawContext[] = [];
...
function main(): void {
  ...
  if (resource === "glossary" && action) {
    const contexts = loadContexts(dir);
    const ctx = contexts.find(
      (c) => c.name.toLowerCase() === action.toLowerCase(),
    );
    if (!ctx) {
